odoo.define('resident_management.button_create_admin_user', async function (require) {
    ...
    var perm_delete_admin_user = await rpc.query({
                                model: 'res.users',
                                method: 'check_perm_create',
                                args: ["perm_delete_admin_user"],
                            });
    ...
    var perm_change_password_admin_user = await rpc.query({
                                model: 'res.users',
                                method : 'check_perm_create',
                                args: ["perm_change_password_admin_user"],
                            });
...
//                else this.$buttons.find('.btn.btn-secondary.o_form_button_delete').hide();
       ...
       _getActionMenuItems: function (state) {
            ...
            if (this.activeActions.delete && perm_delete_admin_user) {
                otherActionItems.push({
                    description: _t("Delete"),
                    callback: () => this._onDeleteRecord(this),
                });
            }
            if (!perm_change_password_admin_user && this.toolbarActions.action) {
